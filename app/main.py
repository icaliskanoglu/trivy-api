...
class ScanRequest(BaseModel):
    image: str = Field(..., description="Docker image to scan (e.g., 'nginx:latest', 'alpine:3.18')")
...
async def scan_image(request: ScanRequest):
    ...
    logger.info(f"Scan parameters - Severity: {request.severity}, Scanners: {request.scanners}")
    ...
    try:
        ...
        if result.returncode != 0 and not scan_result:
            logger.error(f"Trivy scan failed for image {request.image}: {result.stderr}")
            raise HTTPException(
                status_code=500,
        ...
        logger.info(f"Scan completed successfully for {request.image}. Found {vuln_count} vulnerabilities")
        ...
        raise HTTPException(
            status_code=504,
            detail="Scan timeout exceeded (5 minutes)"
        )
    except FileNotFoundError:
        logger.error("Trivy executable not found")
        raise HTTPException(
            status_code=500,
            detail="Trivy not found. Please ensure Trivy is installed."
        )
    except json.JSONDecodeError as e:
        logger.error(f"Failed to parse Trivy JSON output: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to parse Trivy output: {str(e)}"
        )
    except Exception as e:
        logger.error(f"Unexpected error during scan of {request.image}: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
